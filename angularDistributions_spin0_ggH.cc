...
#include "TString.h"
#include "TChain.h"
#include "CMSJHU_AnalysisMacros/MCProductionValidation/include/ScalarPdfFactory_ggH.h"

using namespace RooFit;
using namespace std;

void angularDistributions_spin0_ggH(string cinput, double g1Re=1, double g2Re=0, double g4Re=0, double g1L1Re=0, double g2Im=0, double g4Im=0, double g1L1Im=0, int nbins=80, double mPOLE = 125., int useTaus=0, vector<string> morecinputs = vector<string>()){
  ...
  else if (cinput.find("GG")!=string::npos && cinput.find("GGto")==string::npos){ nToPlot-=6; Vdecay1=RooSpin::kVdecayType_GammaOnshell; Vdecay2=RooSpin::kVdecayType_GammaOnshell; }
  ...
  for (auto it = morecinputs.begin(); it != morecinputs.end(); ++it)
    tree->Add(it->c_str());
  ...
  for (int ev=0; ev<tree->GetEntries(); ev++){
    ...
    if (
      ...
      if (GenLepId[0]==GenLepId[2] && GenLepId[1]==GenLepId[3] && Vdecay1!=RooSpin::kVdecayType_GammaOnshell && Vdecay2!=RooSpin::kVdecayType_GammaOnshell && mPOLE<2*91.1876) continue;
